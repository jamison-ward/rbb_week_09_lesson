...
::: practice
**Practice** Create a time series plot of the GPD per capita (`gdpPercap`) recorded in the `gap_US` data frame by using `geom_line()` to create a line graph.
...
::: practice
**Practice** Building on the code above, visualize the relationship between time and **GPD per capita** from the `gap_US` data frame.
...
::: challenge
**Challenge** Aesthetic mappings can either go in the main `ggplot()` function, or in a specific `geom_*()` function that is added on.
...
::: practice
**Practice** Using the `gap_mini` data frame, create a **population** growth chart with these aesthetic mappings:
...
::: practice
**Practice** We can customize scale breaks on a continuous **y**-axis values with **`scale_y_continuous()`**.
...
::: practice
**Practice** First subset `gapminder` to only the rows containing data for **Uganda:**
...
::: practice
**Practice** Create a time series plot comparing the trends in life expectancy from 1952-2007 for **three countries** in the `gapminder` data frame.

First, subset the data to three countries of your choice:

```{r eval = F, include=FALSE}
my_gap_mini <- gapminder %>% filter(country %in% c("France", "Italy", "Spain"))
```
